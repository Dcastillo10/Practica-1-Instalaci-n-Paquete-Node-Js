...
//Nuevo método post para buscar documentos en nuestra coleccion
router.post('/find', (req, res) => {
  Person.find({ nombre: { $regex: req.body.criteria, $options: "i"}}) //nuestra busqueda se enfocará en el atributo nombre
  //.find() es una promesa si es que existen los datos
 .then((Persons) => res.render("index", { Persons })) //Renderiza la vista index.ejs
 .catch((error) => res.json({message:error})); //si hay algún error nos manda un mensaje de error
});
